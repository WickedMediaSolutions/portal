...
// Handshake
        ...
        if (!handshakeResponse.Accepted)
        {
            ...
            var rejectMessage = !string.IsNullOrEmpty(handshakeResponse.ErrorMessage) ? handshakeResponse.ErrorMessage! : "ROP rejected the Portal handshake.";
        ...
        if (handshakeResponse.ProtocolVersion != ProtocolVersion.Current)
        {
            ...
            return Result<SessionInfo>.Failure(
                ...
                "ROP accepted an incompatible Portal protocol version.");
        }
// Authentication
